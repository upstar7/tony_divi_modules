...
<div xmlns:v="http://rdf.data-vocabulary.org/#">';  
  ...
  elseif ( is_single()){
    ...
    elseif ($category_id == 0) {
      ...
      $titlearchive = $tata->labels->menu_name;
      $urlarchive = get_post_type_archive_link( $post_type );
      $final .= $data.'<a class="breadl" href="'.$urlarchive.'" title="'.$titlearchive.'" rel="v:url" property="v:title">'.$titlearchive.'</a>'.$dataend;}
    // With Comments pages
  ...
  elseif ( is_category() ) {
    ...
    if ( $paged <= 1 )
      ...
      $final .= $data.'<a href="' . get_category_link( $category ) . '" title="See all articles from '.single_cat_title("", false).'" rel="v:url" property="v:title">'.single_cat_title("", false).'</a>'.$dataend;}
  // Page
  ...
  elseif ( is_author() ) {
    ...
    $final .= "Author articles ".$curauth->nickname;}  
  // tags
  elseif ( is_tag() ){
    $final .= "Themed articles ".single_tag_title("",FALSE);}
  // Search
  elseif ( is_search() ) {
    $final .= "Results of your search on \"".get_search_query()."\"";}    
  // Dates
  elseif ( is_date() ) {
    if ( is_day() ) {
      $year = get_year_link('');
      ...
      $final .= $sep."Archive for ".get_the_date();}
    elseif ( is_month() ) {
      $year = get_year_link('');
      $final .= $data.'<a title="'.get_query_var("year").'" href="'.$year.'" rel="v:url" property="v:title">'.get_query_var("year").'</a>'.$dataend;
      $final .= $sep."Archive for ".single_month_title(' ',false);}
    elseif ( is_year() )
      $final .= "Archive for ".get_query_var('year');}
  // 404 page
  elseif ( is_404())
    $final .= "404 Page not found";
  // Other Archives
  elseif ( is_archive() ){
    $posttype = get_post_type();
    $posttypeobject = get_post_type_object( $posttype );
    $taxonomie = get_taxonomy( get_query_var( 'taxonomy' ) );
    $titlearchive = $posttypeobject->labels->menu_name;
    if (!empty($taxonomie))
