...
function TY_Carousel_Testimonial() {
	if(class_exists("ET_Builder_Module")){
		class ET_Builder_Module_Carousel_Testi extends ET_Builder_Module {
			function init() {
				...
				$this->slug            = 'et_pb_slick_slider_TY_testi';
				$this->child_slug      = 'et_pb_slick_slide_TY_testi';
			...
			function shortcode_callback( $atts, $content = null, $function_name ) {
				...
				$output = sprintf(
					'<div class="et_pb_module et_pb_slick_slider_TY_testi">
						<div class="slick-carousel-testi">
							%1$s
						</div> <!-- .et_pb_slides -->
					</div> <!-- .et_pb_slick_slider_TY_testi -->
					',
					$content
				);
		...
		class ET_Builder_Module_Carousel_Testi_Item extends ET_Builder_Module {
			function init() {
				...
				$this->slug                        = 'et_pb_slick_slide_TY_testi';
				$this->type                        = 'child';
				...
				$this->advanced_setting_title_text = esc_html__( 'New Item' );
				$this->settings_text               = esc_html__( 'Item Parameters');
			}
			...
			function get_fields() {
				$fields = array(
					'background_image' => array(
						...
						'description'        => esc_html__( 'Item Image', 'et_builder' ),
					),
					'image_url' => array(
						'label'           => esc_html__( 'Item URL', 'et_builder' ),
						'type'            => 'text',
						'option_category' => 'basic_option',
						'description'     => esc_html__( 'Item URL', 'et_builder' ),
					),
					'name' => array(
						'label'       => esc_html__( 'Last name', 'et_builder' ),
						'type'        => 'text',
						'description' => esc_html__( 'Name to display', 'et_builder' ),
					),
					'fonction' => array(
						'label'       => esc_html__( 'Function', 'et_builder' ),
						'type'        => 'text',
						'description' => esc_html__( 'Function to display', 'et_builder' ),
					),
				);
				return $fields;
			}
		...
		add_action( 'admin_notices', 'TY_admin_notice' );      
		return;
	}
}

add_action('et_builder_ready', 'TY_Carousel_Testimonial');